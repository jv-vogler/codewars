...
function battle(player1, player2) {
  ...
  const numCreaturesP1 = player1.length;
  const numCreaturesP2 = player2.length;

  for (let i = 0; i < Math.max(numCreaturesP1, numCreaturesP2); i++) {
    const creature1 = player1[i];
    const creature2 = player2[i];

    if (!creature1) {
      results.player2.push(creature2)
      continue;
    }

    if (!creature2) {
      results.player1.push(creature1)
      continue;
    }

    const attack1 = creature1[0];
    const health1 = creature1[1];
    
    const attack2 = creature2[0];
    const health2 = creature2[1];

    const damage1 = health1 - attack2;
    const damage2 = health2 - attack1;
